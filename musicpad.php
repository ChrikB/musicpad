...
@media print {
  #title,#subtitle,#title_wrapper,#menuX,#menuX span,.palettes,.thrash_bin{display:none;fill:rgba(255,255,255,0);}
}
...
function MusicPad( targetEl , MenuWrapper ){
	   ...
	   "use strict";
		...
		this.TargetSymbol  = { 
		                          ...
		                          beamObje    : null,
								  dragPoint  : { x : 0 , y : 0 },
                                  Reset      : function(){ var k = this;k.el = "",k.startPos.x = k.startPos.y = k.dragPoint.x = k.dragPoint.y = 0,k.scalable = k.movable = false,k.beamObj = null; }
		},
		this.MainSvgPad    = {   
                           		  SvgPoint   : ""    ,
								  MouseCoord : { x : 0 , y : 0 },
		                          ...
		                          Attributes : [ 
		                                         ...
		                                         // { prop : "height"  , val : self.SVGViewBox.H } ,
												  //{ prop : "preserveAspectRatio"  , val : "xMinYMin meet"},
								  ...
								  Events     : {
								                    mouseleave : function(evt){ self.ResetSymbol( evt ); },
								                    mousemove  : function(evt){ evt.preventDefault();self.MainSvgPad.MouseCoord =  self.cursorPoint(evt,self.MainSvgPad.El); self.MoveSvgIn();self.ScaleSvg();},
												    mouseup    : function(evt){ self.ResetSymbol( evt ); }
								  },
								  Children   : {    
								                    EditArea  : { el:"", El_tag : "svg" , Attributes : [ { prop : "class"    , val : "edit_area_svg"   },
								                    ...
								                    Palette   : { el:"", El_tag : "svg" , Attributes : [ { prop : "class"    , val : "palettes"        },
								                                                                         { prop : "x"        , val : self.SVGViewBox.X },
 								                                                                         { prop : "y"        , val : self.SVGViewBox.Y },
								                                                                         { prop : "width"    , val : self.SVGViewBox.W },
																									     { prop : "height"   , val : self.SVGViewBox.H },
												    ...
												    Stuffs    : { el:"", El_tag : "svg" , Attributes : [ { prop : "class"    , val : "stuffs"          },
								                                                                         { prop : "x"        , val : self.SVGViewBox.X },
 								                                                                         { prop : "y"        , val : self.SVGViewBox.Y },
								                                                                         { prop : "width"    , val : self.SVGViewBox.W },
																									     { prop : "height"   , val : self.SVGViewBox.H },
								  ...
								  Create     : function( targetEl ){
                                                          ...
                                                          for( var a in self.MainSvgPad.Children ){
														  
														                   var child = self.Helpers.createSvgEl( 
																		                                         {
																			   ...
																			   if(self.MainSvgPad.Children[a].hasOwnProperty("Events")){
																			      for(var wa in self.MainSvgPad.Children[a].Events ){
		...
		this.UnitStuffs       = function( parentSvg ){
					  ...
					  function createStuff(){
                					  ...
                					  if( stuffs.length == 0 ){ Ypos = Distance; }
                					  else { Ypos = stuffs[stuffs.length-1].getAttribute("y"); }					  
                					  Ypos = parseInt(Ypos,10) + Distance;
					  
                 					  var Stuff = self.Helpers.createSvgEl( {  svg_type      : "svg" ,
                 					                                           attribute_Arr : [ { prop:"class",val:"stuff"}, { prop:"y",val:Ypos} ,{ prop : "viewBox" , val : self.SVGViewBox.X+" "+self.SVGViewBox.Y+" "+self.SVGViewBox.W+" " +self.SVGViewBox.H }]
                 															}
                 					  ...
                 					  for( var i = 0; i < 5; i++ ){
                                             ...
                                             var newLine = self.Helpers.createSvgEl( {     svg_type      : "line" , 
                 						                                                   attribute_Arr : [ { prop : "class"    , val : "stuff_line"      } ,  
                 																		                     { prop : "x1"       , val : 0                 } ,  
		...
		this.AddSymbol         = function( parentEl , HTMLObject , x , y  ){ 
						          ...
						          var refSvg = self.Helpers.createSvgEl( { svg_type      : "svg" , 
						                                                   attribute_Arr : [ { prop : "class"    , val : "musicSymbol "+HTMLObject.name  } ,  
																		                     { prop : "x"        , val : x                } ,  
																		                     { prop : "y"        , val : y                } ,
                                   ...
                                   var rect = self.Helpers.createSvgEl( {  svg_type      : "rect" , 
						                                                   attribute_Arr : [ { prop : "x"      , val : "0"                      } ,  
																		                     { prop : "y"      , val : "0"                      } ,
																						     { prop : "width"  , val : "100%"                   } ,
																							 { prop : "height" , val : "100%"                   } ,
...
{ prop : "viewBox" , val : self.SVGViewBox.X+" "+self.SVGViewBox.Y+" "+self.SVGViewBox.W+" " +self.SVGViewBox.H }																							 
						          ...
						          var refSvgText = self.Helpers.createSvgEl( {  svg_type      : "text" , 
						                                                        attribute_Arr : [ { prop : "x"      , val : HTMLObject.text.x      } ,  
								  ...
								  if( HTMLObject.text.hasOwnProperty("tr")     ){ refSvgText.setAttribute("transform",HTMLObject.text.tr); }	
								  if( HTMLObject.text.hasOwnProperty("letter") ){
							    	     ...
							    	     var textNode = document.createTextNode(HTMLObject.text.letter);
	                                        ...
	                                        if( n == "zero" || n== "one" || n=="two" ||n=="three"||n=="four"||n=="five"||n=="six"||n=="seven"||n=="eight"||n=="nine"||n=="equal" ){
		...
		this.getSymbol         = function( symbol ){
								...
								this.Symbol_MouseOver   = function( e ){
										                 ...
										                 // if( !self.Helpers.hasClass(symbol,"numbers") ){ 
														  ...
														  //}
										                    //symbol.getElementsByTagName("rect")[0].setAttribute('style', 'fill:rgba(100, 19, 100,0.3);' );
		...
		this.UnitPaletteMenu = function(){
			                   ...
			                   //--var palettemenu = document.createElement("div");
							   //--    palettemenu.setAttribute("class","palette_menu");
	                           var palettemenu =  MenuWrapper;
								   ...
								   for( var o in self.Palette.category ){
								    ...
								    var paletteSVG =  self.UnitPalette( Palettes , self.Palette.category[o_prop] , o_prop );									
									    ...
									    menuLink.addEventListener("click"   ,function(e){    
										                                      var all_palettes = Palettes.childNodes;//.getElementsByTagName("svg");
								  ...
								  //--targetEl.insertBefore(palettemenu, targetEl.firstChild);
		...
		this.UnitPalette    = function( palleteFather , objectArray , clname ){
						       ...
						       var svgPaletteNode = self.Helpers.createSvgEl( { svg_type      : "svg" , 
						                                                        attribute_Arr :[ { prop : "class"   , val : "palette " + clname  },  
																		                         { prop : "x"       , val : "0"                  },
																								 { prop : "y"       , val : "0"                  },
																								 { prop : "width"   , val : self.SVGViewBox.W    },
																								 { prop : "viewBox" , val : self.SVGViewBox.X+" "+self.SVGViewBox.Y+" "+self.SVGViewBox.W+" " +self.SVGViewBox.H }
								...
								var previous = null,Sunit = objectArray,StartXPoing = 40;
                                 ...
                                 for(var k=0;k<Sunit.length;k++){
                  	                  (function(idx){ 
                                  if(previous!=null){ var xX = parseInt( previous.getAttribute("width") , 10 )+parseInt( previous.getAttribute("x") , 10 )+2; }else{var xX=0;}
                                                      if(xX>idx*15){xX = xX;}else{xX=idx*15};
		...
		this.ScaleSvg   = function(){
		                                 if( self.TargetSymbol.el!="" && self.TargetSymbol.scalable ){
		                                  var SvgEl = self.TargetSymbol.el;
										  var w = self.loc.x -self.StartX +  self.StartW ;//+ parseInt(SvgEl.getAttribute("x"),10);
										  var h = self.loc.y -self.StartY +  self.StartH ;//+ parseInt(SvgEl.getAttribute("y"),10);
		...
		this.BeamDestroy= function( beamObj ){  
											   ...
											   for( var m = 0; m < BeLen; m++ ){ 
											                if( self.Beams[m] === FullTree ){ index = m;
															  ...
															  // beamObj.BeamObj.FullTree.Move(0,0,beamObj.BeamObj.FullTree);
															  }
															}
		...
		this.BeamSymbol = function( edit_area , SymbolPath , Direction  , MouseXcor , MouseYcor ){
					  ...
					  function TranceIntel_Beam( par , D , path_x ,path_y , lineSpace){ 
					                ...
					                var pathObj = this;
                    				    ...
                    				    this.g    = self.Helpers.createSvgEl( {      svg_type      : "g" });  EditArea.insertBefore( this.g,EditArea.firstChild);
										...
										this.TreeObject = {
 										                   BeamObj  : { FullTree : null , g0 : null , g1 : null , g2 : null , g3 : null , 
														                SubTree0 : null , SubTree1 : null, SubTree2 : null, SubTree3 : null
																     },
										                   BeamWith : false,
										                   g        : this.g,
										                   path     : null,
														   ...
														   x        : path_x,
														   y        : path_y,
														   D_n      : " -0.5,29.6 c -1.20,-0.8 -3.6,-1.20 -6,-0 -2.80,1.20 -4.80,4 -3.6,6 0.8,2 4,2.40 7.2,1.20 0,-0 0,-0 0.4,-0 2,-1.20 3.6,-3.2 3.2,-5.2 l 0,-31.6 z",
														   //D_r      : " -0.4,-0 0,-32.4 c 0,-1.6 0.8,-2.40 2,-3.6 1.6,-1.20 3.2,-1.6 5.2,-1.6 2.40,-0 3.6,1.20 3.6,2.80 -0,3.2 -4.80,6 -7.60,6 -0.8,-0 -2.40,-0.4 -2.80,-0.8 z",
		                               ...
		                               }//end of for loop trance
									   
									   function Create( obj ){
									   
									         if( obj.hasOwnProperty("path") ){
							obj.el    = self.Helpers.createSvgEl( { svg_type : "path" , attribute_Arr :[{prop:"d",val:"m "+path_x+","+path_y + obj.D_tale } ] });
							obj.path  = obj.el;
											 }									   
									         if( obj.hasOwnProperty("cx") ){
							obj.el    = self.Helpers.createSvgEl( { svg_type : "circle" , attribute_Arr :[{prop:"class",val:""},{prop:"cx",val:obj.cx},{prop:"cy",val:obj.cy},{prop:"r",val:4}] });
											 }
				   ...
				   var MouseEv = { 
				               ...
				               TargetEl     : null,
				               MousePress   : null,
						       ResetIt      : function(){ this.MousePress = this.TargetEl = this.g = this.TreeObj = this.WholeTree = this.Collision = this.CollisionObj = null;},
							   g            : null,
							   TreeObj      : null,
							   WholeTree    : null,
							   Collision    : null,
				   ...
				   function ResetPos( e ){ alert("mouseUp Not In the path");
				   ...
				   function FoundInTree( tree , elem ){
				            ...
				            for( var la = 0; la < 4; la++ ){
					           ...
					           if(  tree.Points["circle"+la].Line.Circle.el             == elem ){ MouseEv.g = la;   MouseEv.TreeObj = tree.Points["circle"+la].Line.Circle;break;}
						       if(  tree.Points["circle"+la].Line.Circle.Line.Circle.el == elem ){ MouseEv.g = la;   MouseEv.TreeObj = tree.Points["circle"+la].Line.Circle.Line.Circle;break;}
					        }
				   ...
				   function Collision( f ){ 
							...
							var cicle_y = parseInt( MouseEv.TargetEl.getAttribute("cy"),10);
				                  ...
				                  for( var ty = 0; ty < self.Beams.length; ty ++ ){
								       ...
								       var other = self.Beams[ ty ].TreeObject.Points["circle" + f ].Line.Circle.el;//.Line.Circle.el;
									   
									   if( other!=MouseEv.TargetEl && MouseEv.WholeTree!= self.Beams[ ty ].TreeObject ){ 
								       if( cicle_x > parseInt( other.getAttribute("cx") ,10) - 20 && cicle_x < parseInt( other.getAttribute("cx") ,10) + 20 &&   
									       cicle_y > parseInt( other.getAttribute("cy") ,10) - 10 && cicle_y < parseInt( other.getAttribute("cy") ,10) + 10
									     ){
											   ...
											   if( self.Beams[ ty ].TreeObject.BeamObj["g0"] === null && self.Beams[ ty ].TreeObject.BeamObj["g1"] === null  &&
												   self.Beams[ ty ].TreeObject.BeamObj["g2"] === null && self.Beams[ ty ].TreeObject.BeamObj["g3"] === null 
												){
...
MusicPad.prototype.Helpers = function() { 
		...
		var Helpers = {
		           createSvgEl : function( svg_obj ){
							 ...
							 if( typeof(svg_obj.attribute_Arr) != "undefined" && svg_obj.attribute_Arr!="" && svg_obj.attribute_Arr.constructor === Array && svg_obj.attribute_Arr.length > 0 ){
                   ...
                   addClass    : function(el, cn)
                   {
                                                         if (!Helpers.hasClass(el, cn)) { 
														   var cla;
															    ...
															    if( el.getAttribute("class")==='' || el.getAttribute("class")=== null){ cla = cn; }
															    else                                                                  { cla = el.getAttribute("class") +' '+cn; }
...
onload=function(){  
 ...
 var MusicEditor = new MusicPad( document.getElementById("musicpad") , document.getElementById("menuX") ); 
...
<BODY>
          ...
          <div id="wrapper" oncontextmenu="/*return false;*/" style="height:auto;">
               ...
               width: 80%;
               padding-bottom: 70%; 
               vertical-align: top; max-width:none;height:auto;margin-top:0px;"> </div>
